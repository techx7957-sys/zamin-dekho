...
if (!process.env.MONGO_URI || !process.env.JWT_SECRET) {
    console.error(
        "❌ CRITICAL ERROR: MONGO_URI or JWT_SECRET is missing in .env!",
    );
    ...
    cors({
        origin: function (origin, callback) {
            ...
            if (
                ...
                callback(
                    new Error(
                        "🚨 CORS Blocked: Origin not trusted by Zamin Dekho",
                    ),
                );
            }
        },
...
const authLimiter = rateLimit({
    ...
    message: {
        success: false,
        message:
            "🚨 Too many login attempts. Please wait 15 minutes to protect your account.",
    },
});
...
app.get("*", (req, res) => {
    ...
    if (req.path.includes("..") || req.path.includes("%00")) {
        return res
            .status(403)
            .send("🚨 Access Denied by Zamin Dekho Firewall.");
    }
    ...
    const inProtectedFolder =
        filePath.includes(path.join(__dirname, "routes")) ||
    ...
    if (isBlocked || inProtectedFolder) {
        return res
            .status(403)
            .send("🚨 Forbidden: Access Denied. Zamin Dekho Security.");
    }
...
app.use((err, req, res, next) => {
    ...
    res.status(500).json({
        success: false,
        message: "Internal Server Error! Engine is protected.",
    });
});
...
if (!process.env.VERCEL) {
    ...
    server.on("error", (err) => {
        if (err.code === "EADDRINUSE") {
            console.log(
                `⚠️ Port ${PORT} is busy... Try running 'killall node' in terminal.`,
            );
        } else {
