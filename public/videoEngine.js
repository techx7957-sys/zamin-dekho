...
function resolvePublisherWaiters(streamId, success) {
    ...
    for (const waiter of matchingWaiters) {
        ...
        try { waiter.resolve(success); } catch (e) { console.warn("⚠️ Publisher waiter resolve failed:", e); }
    }
}
...
function waitForPublisherState(streamId, timeoutMs = 30000) {
    if (publisherLifecycleStreamId === streamId && publisherLifecycleState === "PUBLISHING") return Promise.resolve(true);
    return new Promise(resolve => {
...
let currentZoom = 1.0;
let minZoom = 1.0;
let maxZoom = 10.0; // Upgraded to 10x for Hybrid Mode
...
let gpuAttribs = {}; let blurAttribs = {}; let compositeAttribs = {}; let imageCompositeAttribs = {}; let maskRefineAttribs = {}; let beautyAttribs = {};
let gpuUniforms = {}; let blurUniforms = {}; let compositeUniforms = {}; let imageCompositeUniforms = {}; let maskRefineUniforms = {}; let beautyUniforms = {};
...
async function updateZegoBitrate(bitrate) {
    ...
    try { await engine.setVideoConfig(nextConfig); return true; } catch (error) { return false; }
}
...
function detectCameraCapabilities(track) {
    ...
    try {
        ...
        if (cameraCapabilities.zoom) {
            ...
            maxZoom = 10.0; // Ensure 10x hybrid is unlocked
            currentZoom = cameraSettings.zoom || 1.0;
        } else {
            hasHardwareZoom = false; minZoom = 1.0; maxZoom = 10.0;
        }
        return true;
    } catch (e) {
        hasHardwareZoom = false; minZoom = 1.0; maxZoom = 10.0; return false;
    }
}
...
async function switchOutputProfile(profileName) {
    ...
    try {
        ...
        if (gpuReady) {
            try { destroyRenderTargets(); createRenderTargets(CANVAS_W, CANVAS_H); } catch (e) {}
        }

        if (!!zg && !!publishStreamId && typeof updateZegoBitrate === "function") {
        ...
        return true;
    } catch (e) { return false; } finally { profileSwitchInProgress = false; }
}
...
async function evaluateQuality() {
    ...
    try {
        ...
        if (targetLevel === currentLevel) { badFpsSamples = 0; goodFpsSamples = 0; badNetSamples = 0; goodNetSamples = 0; return; }
        ...
        if (targetLevel < currentLevel) {
            ...
            if (badFpsSamples >= 3 || badNetSamples >= 3) {
                ...
                badFpsSamples = 0; goodFpsSamples = 0; badNetSamples = 0; goodNetSamples = 0;
            }
            return;
        }
    ...
    } catch (error) { } finally { qualityEvaluationRunning = false; }
}
...
async function setZoom(value) {
    ...
    if (hasHardwareZoom) {
        ...
        try {
            ...
            zoomMode = "hybrid"; hardwareZoomApplied = hardZoom; digitalZoom = digiZoom;
        } catch (e) {
            zoomMode = "digital"; hardwareZoomApplied = 1.0; digitalZoom = requestedZoom;
        }
    } else {
        zoomMode = "digital"; hardwareZoomApplied = 1.0; digitalZoom = requestedZoom;
    }
...
function setAutoFrame(enabled) {
    ...
    if (!autoFrameEnabled) {
        autoFrameTargetX = 0; autoFrameTargetY = 0; autoFrameCurrentX = 0; autoFrameCurrentY = 0;
    }
}

function updateAutoFrame() {
    if (!autoFrameEnabled || !lastFaceLandmarks || !isCamOn) {
        autoFrameCurrentX += (0 - autoFrameCurrentX) * 0.12; autoFrameCurrentY += (0 - autoFrameCurrentY) * 0.12;
        return;
    }
    ...
    const faceCenterX = (minX + maxX) / 2.0; const faceCenterY = (minY + maxY) / 2.0;
    const sourceAspect = SOURCE_VIDEO_W / SOURCE_VIDEO_H; const outputAspect = CANVAS_W / CANVAS_H;
    let xOffset = 0, yOffset = 0;
    if (sourceAspect > outputAspect) {
        const visibleWidth = outputAspect / sourceAspect; xOffset = (1.0 - visibleWidth) * 0.5;
    } else {
        const visibleHeight = sourceAspect / outputAspect; yOffset = (1.0 - visibleHeight) * 0.5;
    }
    const invZoom = 1.0 / digitalZoom;
    const transformedX = (faceCenterX - xOffset) * invZoom; const transformedY = (faceCenterY - yOffset) * invZoom;
    const targetX = 0.5 - transformedX; const targetY = 0.5 - transformedY;
    const smoothFactor = 0.08;
    autoFrameCurrentX += (targetX - autoFrameCurrentX) * smoothFactor; autoFrameCurrentY += (targetY - autoFrameCurrentY) * smoothFactor;
}
...
function createZoomSliderUI() {
    ...
    zoomContainer.style.cssText = `position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.7); backdrop-filter: blur(4px); padding: 10px 20px; border-radius: 30px; display: flex; align-items: center; gap: 15px; z-index: 25; box-shadow: 0 4px 15px rgba(0,0,0,0.5);`;
...
window.startCustomZegoEngine = async function (appId, token, roomID, userID, userName) {
    try {
        ...
        // 🔥 FIX: Setup Fallback Server URL to prevent connection timeouts
        const serverUrl = window.ZEGO_SERVER_URL || `wss://webliveroom${appId}-api.zego.im/ws`;
        ...
        zg.on("roomStateUpdate", (callbackRoomID, state, errorCode, extendedData) => {
            if (state === "DISCONNECTED") {
                roomConnectionState = "DISCONNECTED"; roomConnectionError = { errorCode, extendedData };
                publisherLifecycleState = "ROOM_DISCONNECTED"; publisherLifecycleError = { errorCode, extendedData };
                if (typeof stopPerformanceMonitor === "function") stopPerformanceMonitor();
                return;
            }
            if (state === "CONNECTED") {
                if (callbackRoomID !== window.meetingRoomId) return;
                roomConnectionState = "CONNECTED"; roomConnectionError = null; publisherLifecycleError = null;
                const waiters = roomConnectionWaiters.splice(0);
                for (const waiter of waiters) { try { waiter.resolve(true); } catch (e) {} }
                if (typeof startPerformanceMonitor === "function" && zg && localStream && publishStreamId) startPerformanceMonitor();
            }
        });

        zg.on("publisherStateUpdate", (streamID, state, errorCode, extendedData) => {
            if (streamID !== publishStreamId) return;
            publisherLifecycleStreamId = streamID; publisherLifecycleState = state || "UNKNOWN";
            publisherLifecycleError = errorCode && errorCode !== 0 ? { errorCode, extendedData } : null;

            if (errorCode && errorCode !== 0) { resolvePublisherWaiters(streamID, false); return; }
            if (state === "PUBLISHING") { resolvePublisherWaiters(streamID, true); return; }
            if (state === "NO_PUBLISH") { resolvePublisherWaiters(streamID, false); }
        });
        ...
        zg.on("roomStreamUpdate", async (callbackRoomID, updateType, streamList, extendedData) => {
            ...
            if (updateType === "ADD") {
                for (const streamInfo of streamList) {
                    ...
                    try { await zg.startPlayingStream(streamInfo.streamID, { video: remoteVideo, audio: remoteVideo }); } 
                    catch (e) {
                        if (streamInfo.stream && remoteVideo) {
                            try { remoteVideo.srcObject = streamInfo.stream; await remoteVideo.play(); } catch (fallbackError) {}
                        }
                    }
                ...
                if (remoteView.childElementCount === 0) {
                    remoteView.innerHTML = `<div class="text-center" id="waiting-text" style="animation: fadeIn 0.5s ease-out;"><i class="fas fa-user-slash mb-3" style="font-size: 50px; color: rgba(255,255,255,0.1);"></i><p class="text-white-50 small fw-bold">User left the room. Waiting for others...</p></div>`;
                }
            }
        });
        ...
        localStream = await zg.createStream({
            camera: {
                video: true, audio: true, videoQuality: 4, width: initProfile.width, height: initProfile.height,
                frameRate: initProfile.fps, bitrate: initProfile.bitrate, audioBitrate: 64, audioMode: "Speech",
        ...
        // 🔥 FIX: Correct Stream Mapping
        publishStream = localStream;

        // 🔥 FIX: Initialize controls IMMEDIATELY so buttons work even if publish is slow
...
// 🔥 FIX 1: The major syntax error in runHeavyBlur is fixed here!
